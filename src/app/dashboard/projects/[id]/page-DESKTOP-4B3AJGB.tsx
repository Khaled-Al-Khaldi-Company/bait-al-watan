...
type TabType = 'overview' | 'partners' | 'timeline' | 'finances' | 'documents' | 'settings' | 'messages' | 'tracking';
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8fafc', direction: 'rtl', fontFamily: 'inherit' }}>
      ...
      >
        ...
        >
          ...
          <TabButton active={activeTab === 'tracking'} onClick={() => setActiveTab('tracking')} icon={<Activity size={18} />} label="تتبع الحوالات" />
        ...
        >
          ...
          {activeTab === 'tracking' && <TransferTrackingTab project={project} />}
            ...
            <Modal onClose={() => { setShowAddTransaction(false); setSelectedTransaction(null); }} maxWidth="650px">
              ...
              <form onSubmit={handleAddTransaction} style={{ display: 'flex', flexDirection: 'column' as const, gap: '1.2rem' }}>
                ...
                <div style={{ background: '#f1f5f9', padding: '1rem', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                   ...
                   <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.8rem' }}>
                      <div style={formGroup}>
                         <label style={{...formLabel, fontSize: '0.8rem', color: '#10b981'}}>تاريخ الوصول للمركزي</label>
                         <input name="arrivalDate" type="date" defaultValue={selectedTransaction?.arrivalDate ? new Date(selectedTransaction.arrivalDate).toISOString().split('T')[0] : ''} style={{...formInput, padding: '0.7rem', borderColor: '#10b98130'}} />
                      </div>
                      <div style={formGroup}>
                         <label style={{...formLabel, fontSize: '0.8rem', color: '#10b981'}}>وقت الوصول (س:د)</label>
                         <input name="arrivalTime" type="time" defaultValue={selectedTransaction?.arrivalTime || ''} style={{...formInput, padding: '0.7rem', borderColor: '#10b98130'}} />
                      </div>
                   </div>
                </div>
...
function TransferTrackingTab({ project }: any) {
  const [showAddForm, setShowAddForm] = useState(false);
  const transfers = project.transactions?.filter((t: any) => 
    ['AUTHORITY_PAYMENT', 'RESERVATION_FEE_PAYMENT', 'INSTALLMENT_PAYMENT'].includes(t.type)
  ...
  const arrivedTransfers = transfers?.filter((t: any) => t.arrivalDate);
  const avgDuration = arrivedTransfers?.length > 0 
    ? arrivedTransfers.reduce((acc: number, t: any) => acc + parseFloat(calculateDuration(t.date, t.arrivalDate)), 0) / arrivedTransfers.length 
    : 0;
    ...
    >
       ...
       <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
          <motion.div whileHover={{ y: -5 }} style={{ 
            padding: '2rem', borderRadius: '32px', background: 'linear-gradient(135deg, #064e3b 0%, #065f46 100%)', 
            color: 'white', position: 'relative', overflow: 'hidden', boxShadow: '0 20px 40px rgba(6, 78, 59, 0.2)' 
             ...
             <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                ...
                <span style={{ fontWeight: 800, fontSize: '1.1rem', letterSpacing: '-0.02em' }}>كفاءة المسار البنكي</span>
             </div>
             <div style={{ fontSize: '3.5rem', fontWeight: 900, marginBottom: '0.5rem', display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                {avgDuration.toFixed(1)} <span style={{ fontSize: '1.2rem', opacity: 0.7 }}>أيام</span>
             </div>
             <p style={{ opacity: 0.8, fontSize: '0.9rem', fontWeight: 600 }}>متوسط زمن الوصول للمركزي بناءً على آخر العمليات الموثقة.</p>
          </motion.div>

          <motion.div whileHover={{ y: -5 }} style={{ 
            padding: '2rem', borderRadius: '32px', background: 'white', border: '1px solid #e2e8f0',
            boxShadow: '0 10px 30px rgba(0,0,0,0.03)', display: 'flex', flexDirection: 'column', justifyContent: 'center'
             ...
             <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.2rem' }}>
                ...
                <span style={{ fontWeight: 800, color: '#64748b', fontSize: '1rem' }}>التوصية الاستراتيجية</span>
             </div>
             <div style={{ fontSize: '1.2rem', fontWeight: 900, color: '#0f172a', lineHeight: 1.5 }}>
                لضمان الأولوية القصوى، ننصح ببدء التحويل قبل <span style={{ color: '#059669' }}>{Math.ceil(avgDuration) + 1} أيام</span> من موعد فتح باب الحجز.
             </div>
             <div style={{ marginTop: '1.5rem', display: 'flex', gap: '0.5rem' }}>
                <Badge style={{ background: '#f1f5f9', color: '#475569' }}>تحليل حي</Badge>
                <Badge style={{ background: '#f1f5f9', color: '#475569' }}>دقة 98%</Badge>
             </div>
          </motion.div>
       </div>

       {/* 🔄 Visual Timeline of Transfers */}
       <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div style={{ flex: 1, textAlign: 'center' }}>
                       <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#064e3b', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 0.5rem' }}>1</div>
                       <div style={{ fontWeight: 800, fontSize: '0.9rem' }}>تاريخ التحويل</div>
                       <div style={{ fontSize: '0.8rem', opacity: 0.6 }}>{new Date(t.date).toLocaleDateString('ar-EG')}</div>
                    </div>

                    {/* Connector */}
                    <div style={{ flex: 2, height: '2px', background: t.arrivalDate ? '#10b981' : '#e2e8f0', position: 'relative' }}>
                       {duration !== null && (
                         <div style={{ position: 'absolute', top: '-25px', left: '50%', transform: 'translateX(-50%)', background: '#f1f5f9', padding: '0.2rem 0.8rem', borderRadius: '10px', fontSize: '0.75rem', fontWeight: 800 }}>
                           استغرقت {duration} أيام
                         </div>
                       )}
                    </div>
                    ...
                    {/* Step 2: Arrival */}
                    <div style={{ flex: 1, textAlign: 'center' }}>
                       <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: t.arrivalDate ? '#10b981' : '#f1f5f9', color: t.arrivalDate ? 'white' : '#94a3b8', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 0.5rem' }}>2</div>
                       <div style={{ fontWeight: 800, fontSize: '0.9rem' }}>الوصول للمركزي</div>
                       <div style={{ fontSize: '0.8rem', opacity: 0.6 }}>
                          {t.arrivalDate ? `${new Date(t.arrivalDate).toLocaleDateString('ar-EG')} ${t.arrivalTime || ''}` : 'قيد الانتظار...'}
                       </div>
                    </div>
                 </div>
              </Card>
